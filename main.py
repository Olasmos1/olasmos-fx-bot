...
def fetch_twelve(symbol, interval, count=100):
    try:
        ...
        if "values" not in data:
            logger.warning(f"TwelveData no values for {symbol}: {data.get('message','')}")
            return None
...
def fetch_yahoo(symbol, interval, count=100):
    try:
        yf_map = {
        ...
        range_map = {
            "1min": "2d", "5min": "60d", "15min": "60d",
            "30min": "60d", "1h": "6mo", "4h": "6mo",
        }
...
def download_model(filename):
    try:
        ...
        if resp.status_code == 200:
            ...
            return path
        logger.warning(f"Download failed {filename}: HTTP {resp.status_code}")
        return None
    except Exception as e:
    ...
    for symbol in PAIRS:
        try:
            ...
            if mp and sp and fp:
                ...
                logger.warning(f"⚠️ {symbol} model files missing")
        except Exception as e:
...
def add_smc(df):
    ...
    df.drop(columns=["Prev_High_20", "Prev_Low_20", "Swing_High_10", "Swing_Low_10"],
...
def get_news(symbol):
    try:
        ...
        resp = requests.get(
            ...
            params={"q": kw.get(symbol, "forex"), "apiKey": NEWS_API_KEY,
...
async def generate_signal(symbol):
    ...
    ai_boost = 15 if ai_dir == direction and ai_conf >= 0.55 else 0
        ...
        try:
            ...
            if sig["tp2_hit"] and not sig["tp3_hit"]:
                if (d == "buy" and price >= sig["tp3"]) or (d == "sell" and price <= sig["tp3"]):
                    ...
                    daily_trades.append({"setup": sig["setup_num"], "symbol": symbol,
                        "direction": d, "entry": sig["entry"], "pips": pips,
                        "result": "tp", "session": sig["session"]})
...
async def send_daily_summary(bot):
    ...
    for sess_key in sessions_order:
        ...
        for t in grouped[sess_key]:
            ...
            if t["result"] == "tp":
                status = f"TP +{t['pips']} pips 🟢"
            elif t["result"] == "sl":
                status = f"SL -{t['pips']} pips 🔴"
            else:
                status = "Cancelled 🟡"
    ...
    msg += f"With 0.1 lot: {'gain' if total_pips >= 0 else 'lose'} ~${lot_gain:.0f}"
...
async def cmd_debug(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for symbol in PAIRS:
        try:
            df = get_candles(symbol, "5min", 150)
            if df is None:
                await update.message.reply_text(f"{symbol}: No data from any source!")
                continue
            df = add_indicators(df)
            df = add_smc(df)
            df.dropna(inplace=True)
            ...
            direction = "BUY" if bull > bear+1 else "SELL" if bear > bull+1 else "NONE"
            await update.message.reply_text(
                f"{symbol}\n"
                f"Data: OK ({len(df)} candles)\n"
                f"ADX: {adx:.1f} (need 15+)\n"
                f"RSI: {rsi:.1f}\n"
                f"MACD: {'UP' if macd_h > 0 else 'DOWN'}\n"
                f"Bias: {bias}\n"
                f"Bull: {bull} | Bear: {bear}\n"
                f"Direction: {direction}\n"
                f"AI: {'loaded' if symbol in models else 'not loaded'}"
            )
        except Exception as e:
            await update.message.reply_text(f"{symbol}: Error — {str(e)[:80]}")
...
async def scan_loop(bot):
    ...
    await bot.send_message(
        ...
        text=(
            ...
            f"Pairs: {', '.join(PAIRS)}\n"
            f"AI Models: {len(models)}/4 loaded\n"
            "Scalper mode active\n\n"
            ...
            for symbol in PAIRS:
                ...
                try:
                    ...
                    if sig:
                        ...
                        logger.info(f"Signal sent: {symbol} {sig['direction']} {sig['confidence']:.0f}%")
