...
RISK_PERCENT = {
    "EURUSD": 2.0, "GBPUSD": 2.0,
    "USDJPY": 2.0, "XAUUSD": 1.0,
}
TP1_RR = 1.5
TP2_RR = 2.5
TP3_RR = 4.0
...
def calculate_risk(symbol, direction, entry, atr):
    ...
    if direction=="buy":
        sl=entry-sl_dist; tp1=entry+sl_dist*TP1_RR
        tp2=entry+sl_dist*TP2_RR; tp3=entry+sl_dist*TP3_RR
    else:
        sl=entry+sl_dist; tp1=entry-sl_dist*TP1_RR
        tp2=entry-sl_dist*TP2_RR; tp3=entry-sl_dist*TP3_RR
    sl_pips  = sl_dist/pip
...
def detect_pattern(df):
    ...
    body=abs(cl-o); rng=h-l+1e-10
    lower=min(o,cl)-l; upper=h-max(o,cl)
...
def get_news(symbol):
    try:
        ...
        headlines = [a["title"].lower()
                     for a in data.get("articles",[])]
        high_impact = any(w in h for h in headlines
                         for w in ["rate decision","nfp",
                                   "cpi","fomc"])
...
async def generate_signal(symbol):
    ...
    min_conf = (MIN_CONFIDENCE_PASSIVE if mode=="passive"
...
async def send_daily_summary(bot):
    ...
    for sess_key in sessions_order:
        if sess_key not in grouped: continue
    ...
    if cancelled>0: msg += f"🟡 {cancelled} cancelled\n"
    msg += (f"\n📈 *FINAL RESULT: "
            f"{sign}{total_pips:.0f} PIPS ({result_w})*\n\n")
    msg += (f"💰 With 0.1 lot per setup, you could "
            f"{'gain' if total_pips>=0 else 'lose'} "
    ...
    while True:
        try:
            ...
            if (now_utc.hour==18 and
                    now_utc.minute==0 and daily_trades):
                await send_daily_summary(bot)
