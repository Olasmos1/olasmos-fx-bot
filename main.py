# ============================================
# OLASMOS FX BOT — COMPLETE VERSION
# SMC + AI Signal Bot with Daily Summary
# Runs 24/7 on Render
# ============================================
...
bot_paused    = False
active_trades = {}
signal_stats  = {p: {"wins": 0, "losses": 0, "total": 0}
                 for p in PAIRS}
models        = {}
scalers       = {}
feature_cols  = {}
...
@app_flask.route('/')
def home():
    return """<html><body style='background:#0a0f1a;
...
def add_indicators(df):
    ...
    df["EMA_21"]      = ta.trend.EMAIndicator(
                            close, 21).ema_indicator()
    df["EMA_50"]      = ta.trend.EMAIndicator(
                            close, 50).ema_indicator()
    df["EMA_200"]     = ta.trend.EMAIndicator(
                            close, 200).ema_indicator()
...
def add_smc(df):
    ...
    for i in range(2, len(df)):
        ...
        bull_ob.append(
            1 if df["Close"].iloc[i-1] < df["Open"].iloc[i-1]
...
def calculate_risk(symbol, direction, entry, atr):
    ...
    risk_usd = min(
        10.0 * RISK_PERCENT.get(symbol, 2.0) / 100, 0.20)
    ...
    sl_pips  = sl_dist / pip
    lot_size = max(
        round(min(risk_usd / (sl_pips * 10 * 0.01), 0.10), 2),
        0.01)
...
async def generate_signal(symbol):
    ...
    min_conf = (MIN_CONFIDENCE_PASSIVE
                if mode == "passive"
                else MIN_CONFIDENCE_ACTIVE)
    ...
    if symbol == "XAUUSD" and sessions == ["asian"]:
        ...
        if df_c is not None:
            ...
            if len(df_c) > 1:
                if (float(df_c["ATR"].iloc[-1]) <
                        float(df_c["ATR"].mean()) * 1.5):
                    return None
    ...
    ai_boost   = (15 if ai_dir == direction
                  and ai_conf >= AI_CONFIDENCE_THRESH else 0)
    base_score = min(
        (max(bull, bear) / (bull + bear + 1e-10)) * 85, 85)
    confidence = min(base_score + ai_boost, 100)
    if confidence < min_conf: return None

    atr_rng   = float(last["High"] - last["Low"])
    fast_mode = (rsi >= 80 or rsi <= 20
                 or (atr > 0 and atr_rng > atr * 2))
    ...
    sess_str = ("+".join(s.title() for s in sessions)
    ...
    if (float(last.get("Bullish_BOS", 0)) > 0
            or float(last.get("Bearish_BOS", 0)) > 0):
        reasons.append("BOS✅")
    if float(last.get("FVG_Present", 0)) > 0:
        reasons.append("FVG✅")
    if (float(last.get("Liq_Grab_Low",  0)) > 0
            or float(last.get("Liq_Grab_High", 0)) > 0):
...
async def send_daily_summary(bot):
    ...
    if not daily_trades:
        await bot.send_message(
            chat_id=CHAT_ID,
            text="📊 No completed trades today yet!"
        )
        return
    ...
    sessions_order = ["asian", "london",
                      "new_york", "overlap", "none"]

    # Count results
    wins      = sum(1 for t in daily_trades
                    if t["result"] == "tp")
    losses    = sum(1 for t in daily_trades
                    if t["result"] == "sl")
    cancelled = sum(1 for t in daily_trades
                    if t["result"] == "cancelled")
    total     = len(daily_trades)
    total_pips = sum(
        t["pips"]  if t["result"] == "tp"
        else -t["pips"] if t["result"] == "sl"
        else 0
    ...
    result_word = "PROFIT" if total_pips >= 0 else "LOSS"
    ...
    msg += (f"\n📈 *FINAL RESULT: "
            f"{sign}{total_pips:.0f} PIPS "
            f"({result_word})*\n\n")
    msg += (f"💰 With 0.1 lot per setup, "
            f"you could {'gain' if total_pips >= 0 else 'lose'} "
            f"around ${lot_gain:.0f}.")
    ...
    daily_trades  = []
    setup_counter = 0
    logger.info("✅ Daily summary sent and trades reset!")
...
async def cmd_status(update: Update,
                     ctx: ContextTypes.DEFAULT_TYPE):
    mode = get_bot_mode()
    sess = (", ".join(s.title() for s in get_sessions())
            or "None")
    me   = {"sleep": "💤", "passive": "😴",
            "active": "✅"}.get(mode, "❓")
    await update.message.reply_text(
        ...
        f"📋 Today's trades: {len(daily_trades)}\n"
        f"Weekend: {'🔴 Yes' if is_weekend() else '✅ No'}",
        parse_mode="Markdown"
    )

@auth
async def cmd_stats(update: Update,
                    ctx: ContextTypes.DEFAULT_TYPE):
    lines = ["📈 *Statistics*\n━━━━━━━━━━━━━━"]
    tw, tl = 0, 0
...
@auth
async def cmd_risk(update: Update,
                   ctx: ContextTypes.DEFAULT_TYPE):
    lines = ["💰 *Risk Settings*\n━━━━━━━━━━━━━━"]
    for p in PAIRS:
        r = RISK_PERCENT[p]
...
@auth
async def cmd_session(update: Update,
                      ctx: ContextTypes.DEFAULT_TYPE):
    sess = (", ".join(s.title() for s in get_sessions())
            or "None")
    mode = get_bot_mode()
...
@auth
async def cmd_pairs(update: Update,
                    ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📋 *Pairs*\n" +
        "".join(f"• {p}\n" for p in PAIRS) +
...
@auth
async def cmd_pause(update: Update,
                    ctx: ContextTypes.DEFAULT_TYPE):
    global bot_paused
    bot_paused = True
    await update.message.reply_text(
        "⏸ Paused. /resume to restart.")

@auth
async def cmd_resume(update: Update,
                     ctx: ContextTypes.DEFAULT_TYPE):
    global bot_paused
    bot_paused = False
...
@auth
async def cmd_scan(update: Update,
                   ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if found == 0:
        await update.message.reply_text(
            "✅ Scan done — no signals right now.")
...
async def scan_loop(bot):
    ...
    await bot.send_message(
        ...
        text=(
            ...
            f"Pairs: {', '.join(PAIRS)}\n"
            f"AI Models: "
            f"{'✅ '+str(len(models))+'/4' if models else '⏳ Loading...'}\n"
            "Scalper mode active 🎯\n\n"
            "Schedule (Nigerian WAT):\n"
            "✅ Active:  6:30AM–7:00PM\n"
            "😴 Passive: 3:00AM–6:30AM\n"
            "💤 Sleep:   7:00PM–3:00AM\n\n"
            "New: /summary for daily report!"
        ),
        parse_mode="Markdown"
    )
    ...
    while True:
        try:
            ...
            interval = (SCAN_INTERVAL_PASSIVE
                        if mode == "passive"
                        else SCAN_INTERVAL_ACTIVE)

            # Auto daily summary at 7PM WAT (18:00 UTC)
            now_utc = utc_now()
            if (now_utc.hour == 18 and
                    now_utc.minute == 0 and
                    daily_trades):
                await send_daily_summary(bot)
